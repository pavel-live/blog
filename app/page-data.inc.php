...
Class PageData {
  ...
  static function get_thumbnail($file_path) {
    $thumbnails = array_keys(Helpers::list_files($file_path, '/thumb\.(gif|jpg|png|jpeg|svg)$/i', false));
  ...
  static function get_thumbnail_render($file_path) {
    $thumbnails = array_keys(Helpers::list_files($file_path, '/thumb\.(gif|jpg|png|jpeg|svg)$/i', false));
	...
	$render_path = preg_replace($content_folder_reg, '', $file_path);
    return (!empty($thumbnails)) ? $render_path.'/'.$thumbnails[0] : false;
  }
  ...
  static function in_route($base_url, $permalink) {
    $base_path = preg_replace('/^[^\/]+/', '', $base_url);
    if($permalink == 'index') {
      return ('/' == $_SERVER['REQUEST_URI']);
    } else {
      return eregi($base_path.'/'.$permalink, $_SERVER['REQUEST_URI']);
    }
  }
  ...
  static function create_vars($page) {
  	#root_content
  	$page->data['@root_content'] =  substr(Config::$content_folder,1);
    # @file_path
    ...
    # @thumb_render
    $page->thumb_render = self::get_thumbnail_render($page->file_path);
    ...
    # @in_route
    $page->in_route = self::in_route($page->data['@base_url'], $page->data['@permalink']);
    ...
    #template
    $page->data['@template'] = self::$template = $page->template_name;
    
    #link
    if (isset($page->data['@link'])) { self::$link = trim($page->data['@link']); $page->data['@bypass_cache'] = 1; } else self::$link = false;
    ...
    #
    # $query
    if(is_array($_GET) && count($_GET)>1 && preg_match('/[\.\?\&][^\/]+$/', $_SERVER['REQUEST_URI'])) {
    	$temp_var_for_query = urldecode($_SERVER['REQUEST_URI']);
    	$page->query = ereg_replace($page->data['@permalink'], '',$temp_var_for_query);
    	// print_r($_GET);
    }
    #
    #
	...
	# @language -postfix (first slug) for multi-language
	if (isset($page->data['@multi-language']))
		{
			$elements_of_url = explode("/",$_SERVER['REQUEST_URI']);
			$page->language = self::$language = '-'.$elements_of_url[1];
		}
	else $page->language = '';
  ...
  static function create_asset_collections($page) {
    ...
    # $video
    $page->video = Helpers::list_files($page->file_path, '/\.(mov|mp4|m4v|avi)$/i', false);
    
    # $archives
    $page->archives = Helpers::list_files($page->file_path, '/\.(zip|rar)$/i', false);
    
    # $grafics
    $page->grafics = Helpers::list_files($page->file_path, '/\.(ai|eps|pdf)$/i', false);
    
	# $files
    $page->files = Helpers::list_files($page->file_path, '/\.(ai|eps|pdf|zip|rar|doc|docx|rtf|xls|xlsx|csv|ppt|pptx|key|keynote|numbers|pages)$/i', false);
    
    # $documents
    $page->documents = Helpers::list_files($page->file_path, '/\.(doc|docx|rtf|xls|xlsx|csv|ppt|pptx|key|keynote|numbers|pages|pdf)$/i', false);
    
    # $mac_documents
    $page->mac_documents = Helpers::list_files($page->file_path, '/\.(rtf|csv|key|keynote|numbers|pages|pdf)$/i', false);
    
    # $win_documents
    $page->win_documents = Helpers::list_files($page->file_path, '/\.(doc|docx|rtf|xls|xlsx|csv|ppt|pptx|pdf)$/i', false);
