...
Class Helpers {
  ...
  static function file_path_to_url($file_path) {
    ...
    $url = preg_replace(array('/\d+?\./', $content_folder_reg), '', $file_path);
  ...
  #######
  #
  static function list_all_files($dir, $regex, $folders_only = false, $replace_part_of_index = false) {
    ...
    foreach(self::file_cache($dir) as $file) {
      ...
      if(preg_match($regex, $file['file_name'])) {
      	...
      	if($file['is_folder']) $files = $files + Helpers::list_all_files($file['path'], $regex, $folders_only, $replace_part_of_index);
      	
        # if $folders_only is true and the file is not a folder, skip it
        if($folders_only && !$file['is_folder']) continue;
        # otherwise, add file to results list
        
	    $name_array_item = $file['path'];
	    
	    if ($replace_part_of_index)
		    {
		        $reg_dir = ereg_replace('\.', '\.', $replace_part_of_index);
		        $reg_dir = ereg_replace('\/', '\/', $reg_dir);
		        
		        $name_array_item = ereg_replace($reg_dir, '', $file['path']);
		    }
        
        $files[$name_array_item] = $file['path'];
      }
    }
