...
Class Stacey {
  ...
  function handle_redirects() {
   ...
   function php_fixes() {
    # in PHP/5.3.0 they added a requisite for setting a default timezone, this should be handled via the php.ini,
    # but as we cannot rely on this, we have to set a default timezone ourselves
  ...
  function create_page($file_path) {
    ...
    if(!$current_page_template_file) {
      $current_page_template_file = Page::template_file("default");
    }
    # render page
  ...
  function __construct($get) {
    ...
    try {
      ...
      if($e->getMessage() == "404") {
        ...
        if(file_exists(Config::$content_folder.'/error')) {
          $this->route = 'error';
          $this->create_page(Config::$content_folder.'/error');
        }
        else {
